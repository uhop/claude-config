...
const defaultBranch =
  run('git', ['symbolic-ref', '--short', 'refs/remotes/origin/HEAD'])?.replace(/^origin\//, '') ??
...
// --- retired artifacts (probe every item — the 2026-07-17 miss class) --------
{
  ...
  const globalSkills = new Set(
    list(path.relative(root, path.join(homedir(), '.claude', 'skills'))).length
...
// --- package.json surfaces ----------------------------------------------------
{
  ...
  facts.pkg = {
    ...
    keywords_count: pkg.keywords?.length ?? 0,
    ...
    bin_targets: Object.values(bins).map(target => ({
      target,
      git_mode: run('git', ['ls-files', '-s', target])?.split(' ')[0] ?? null
    }))
  };
}
...
// --- src sidecar pairing --------------------------------------------------------
{
  ...
  if (!existsSync(srcDir)) facts.sidecars = {src_dir: false};
    ...
    const missingDirective = js.filter(
      f =>
        dts.has(f.replace(/\.js$/, '.d.ts')) &&
...
facts.git = {
  ...
  submodules: [...(read('.gitmodules') ?? '').matchAll(/path\s*=\s*(.+)\n\s*url\s*=\s*(.+)/g)].map(
    m => ({path: m[1].trim(), url: m[2].trim()})
  )
};

// --- CI workflows ---------------------------------------------------------------
facts.workflows = list('.github/workflows')
...
if (wikiDir) {
  ...
  if (facts.wiki.search_index_present) {
    ...
    facts.wiki.pages_newer_than_search_index = list(wikiDir).filter(
      f => f.endsWith('.md') && statSync(path.join(root, wikiDir, f)).mtimeMs > indexMtime
    );
  }
