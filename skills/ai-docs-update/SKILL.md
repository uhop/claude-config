---
name: ai-docs-update
description: Update AI-facing docs (llms.txt, llms-full.txt, ARCHITECTURE.md, AGENTS.md) after API or structural changes. Use when the user invokes /ai-docs-update, changes the public API / module layout / exports, or asks to refresh the machine-readable docs. Companion to /release-check.
---
...
7. Update `AGENTS.md` if critical rules, commands, or the architecture quick
   reference changed.
8. If the retired mirror files (`.windsurfrules` / `.cursorrules` /
   `.clinerules`) are present, **remove them** — do NOT sync or recreate them.
   `AGENTS.md` is the single cross-tool source (mirrors retired fleet-wide
   2026-07-04). Same for an uppercase `.github/COPILOT-INSTRUCTIONS.md`:
   replace with a lowercase `copilot-instructions.md` pointer.
