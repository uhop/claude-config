...
const opts = {tool: null, brew: null, apt: null, dnf: null, sha: '<pending>', out: null, reverse: false};
for (const arg of process.argv.slice(2)) {
  ...
  switch (flag) {
    ...
    case '--brew': opts.brew = value; break;
    case '--apt': opts.apt = value; break;
    case '--dnf': opts.dnf = value; break;
    case '--sha': opts.sha = value; break;
    case '--out': opts.out = value; break;
    case '--reverse': opts.reverse = true; break;
    case '--help': case '-h':
      console.log('Usage: make-migration.mjs --tool=T --brew=B [--apt=A] [--dnf=D] [--sha=SHA] [--out=FILE]');
      process.exit(0);
    default: fail(2, `unknown option: ${arg}`);
  }
}
if (opts.reverse) fail(2, 'reverse (brew→native) has no precedent-tested template — adapt by hand per SKILL.md § Reverse direction');
if (!opts.tool || !opts.brew) fail(2, '--tool and --brew are required');
if (!opts.apt && !opts.dnf) fail(2, 'at least one of --apt / --dnf is required (the native package being replaced)');
const out = opts.out ?? `/tmp/${opts.tool}-migrate.sh`;

const template = readFileSync(path.join(path.dirname(fileURLToPath(import.meta.url)), 'migrate.sh.tmpl'), 'utf8');
...
console.log(`${out} — generated, chmod +x, bash -n clean. Review it, then: playbash put linux,mac --self ${out} ${out}`);
