...
if (!['prepare', 'resolve', 'release'].includes(command)) fail(2, `unknown command: ${command}\n${usage}`);
if (!KINDS.includes(kind)) fail(2, `kind must be one of ${KINDS.join(', ')}\n${usage}`);

const opts = {limit: 25, claim: false, holder: null, ttl: null, scan: null, out: null,
  worksheet: null, decisions: null, label: null, dryRun: false};
for (const arg of rest) {
  ...
  switch (flag) {
    ...
    case '--claim': opts.claim = true; break;
    case '--holder': opts.holder = value; break;
    case '--ttl': opts.ttl = +value; break;
    case '--scan': opts.scan = value ? +value : 0.1; break;
...
if (!Number.isInteger(opts.limit) || opts.limit < 1 || opts.limit > 100) fail(2, '--limit must be 1..100');
...
const brief = (context, id) => {
  ...
  return {record_id: id, file_path: record.file_path, title: record.title,
    type: record.type, status: record.status, updated: record.updated, summary: record.summary};
};
...
const bodyHead = async path => {
  try {
    ...
    const start = typeof text === 'string' && text.startsWith('---\n') ? text.indexOf('\n---\n', 4) + 5 : 0;
...
const prepare = async () => {
  ...
  let items, holder = null, claimed = false, totalPending;
  if (opts.claim) {
    holder = opts.holder ?? `triage-${kind}-${new Date().toISOString().slice(0, 10)}-${Math.random().toString(36).slice(2, 7)}`;
    const response = await api('POST', '/suggestions/claim?expand=context', {
      kind, holder, limit: opts.limit, ...(opts.ttl ? {ttl_seconds: opts.ttl} : {})
    });
    items = response.items;
    claimed = response.claimed > 0;
    totalPending = response.remaining_pending + response.claimed;
  } else {
    const response = await api('GET', `/suggestions?kind=${kind}&status=pending&limit=${opts.limit}&expand=context`);
    items = response.items;
    totalPending = response.total;
  }

  const worksheet = {kind, generated_at: new Date().toISOString(), claimed,
    holder: claimed ? holder : null, total_pending: totalPending, items: [], decisions_template: {}};

  if (kind === 'new_tag') {
    const groups = new Map();
    for (const item of items) {
      const tag = item.payload.tag;
      if (!groups.has(tag)) groups.set(tag, {tag, taxonomy: item.context?.tag ?? null, records: []});
      const record = brief(item.context, item.payload.record_id);
      groups.get(tag).records.push({suggestion_id: item.id, ...record,
    ...
    worksheet.items = items.map(item => {
      const record = brief(item.context, item.payload.record_id);
      return {id: item.id, tag: item.payload.tag,
        ...
        record: {...record, ...(record.file_path ? {path_moved: moved(item.payload.file_path, record)} : {})}};
    });
    ...
    worksheet.items = items.map(item => {
      ...
      return {id: item.id, snippet: item.payload.context,
        from: {...from, ...(from.file_path ? {path_moved: moved(item.payload.from_path, from)} : {})},
    ...
    const sources = [...new Set(worksheet.items.filter(i => !i.from.record_missing).map(i => i.from.record_id))];
    const fmMaps = new Map();
    await pool(sources.map(id => async () => {
      try {
    ...
    worksheet.items = await pool(items.map(item => async () => {
      const a = brief(item.context, item.payload.a_record);
      const b = brief(item.context, item.payload.b_record);
      return {id: item.id, distance: item.payload.distance,
        a: {...a, ...(a.file_path ? {path_moved: moved(item.payload.a_path, a), body_head: await bodyHead(a.file_path)} : {})},
        b: {...b, ...(b.file_path ? {path_moved: moved(item.payload.b_path, b), body_head: await bodyHead(b.file_path)} : {})}};
    }));
    for (const item of worksheet.items) worksheet.decisions_template[item.id] = null;
  }
  ...
  if (opts.out) {
    ...
    console.log(`worksheet: ${opts.out} — ${worksheet.items.length} item(s), ${totalPending} pending total${claimed ? `, claimed by ${holder}` : ''}`);
...
const readDecisions = () => {
  ...
  return raw && typeof raw === 'object' && !Array.isArray(raw) && raw.decisions ? raw.decisions : raw;
};

const isSkip = decision => decision === null || decision === undefined || decision === 'skip' || decision === 'defer';
...
const resolve = async () => {
  ...
  const label = worksheet.holder ?? opts.label ?? `vault-triage-${new Date().toISOString().slice(0, 10)}`;

  const keys = kind === 'new_tag'
  ...
  if (unknown.length) fail(3, `decisions reference keys not in the worksheet: ${unknown.join(', ')}`);

  // validate everything before the first write — a rejected decisions file
  // must leave the queue untouched
  const plan = {batch: [], reopen: [], tagOps: [], fmPatches: [], report: {
    kind, resolved_by: label, accepted: 0, rejected: 0, skipped: 0, failures: [],
    taxonomy_added: [], aliased: [], tags_stripped: [], related_added: [],
    merge_candidates: [], contradictions: []
  }};

  const skipItem = item => {
    ++plan.report.skipped;
    if (worksheet.claimed) plan.reopen.push(...(kind === 'new_tag' ? item.records.map(r => r.suggestion_id) : [item.id]));
  };

  if (kind === 'new_tag') {
    for (const [tag, item] of keys) {
      const decision = decisions[tag];
      if (isSkip(decision)) { skipItem(item); continue; }
      const action = decision.action;
      if (action === 'taxonomy') plan.tagOps.push({kind: 'taxonomy', tag, description: decision.description, item});
    ...
    if (blocked.length) fail(3, `accepting tags not in the taxonomy — add via the new_tag flow first, or defer:\n  ${blocked.join('\n  ')}`);
  } else if (kind === 'edge_type') {
    for (const [id, item] of keys) {
      const decision = decisions[id];
      if (isSkip(decision)) { skipItem(item); continue; }
      if (decision === 'reject' || decision === 'cites') plan.batch.push({id, decision: 'reject'});
      else if (EDGE_TYPES.includes(decision)) plan.batch.push({id, decision: 'accept', edge_type: decision});
      else fail(3, `unknown edge decision "${decision}" for ${id} (valid: ${EDGE_TYPES.join(', ')}, reject, cites, skip)`);
    }
  } else {
    for (const [id, item] of keys) {
      const decision = decisions[id];
      ...
      if (decision === 'merge-candidate') {
        plan.report.merge_candidates.push({id, a: item.a.file_path, b: item.b.file_path, distance: item.distance});
        skipItem(item);
        --plan.report.skipped;
      } else if (decision === 'reject') plan.batch.push({id, decision: 'reject'});
      else if (decision === 'related') {
        if (item.a.record_missing || item.b.record_missing) fail(3, `related decision for ${id}: a record no longer exists`);
        plan.fmPatches.push({record: item.a.record_id, value: `[[${pathKey(item.b.file_path)}]]`, id});
        plan.fmPatches.push({record: item.b.record_id, value: `[[${pathKey(item.a.file_path)}]]`, id});
        plan.batch.push({id, decision: 'accept'});
        ...
        plan.report.contradictions.push({id, a: item.a.file_path, b: item.b.file_path, note: decision.note ?? ''});
  ...
  const failure = (where, err) => plan.report.failures.push({where, status: err.status, code: err.code, message: err.message});
  ...
  for (const op of plan.tagOps) {
    ...
    try {
      if (op.kind === 'taxonomy' || op.kind === 'alias') {
        try {
          const response = op.kind === 'taxonomy'
            ? await api('POST', '/tags/taxonomy', {tag: op.tag, ...(op.description ? {description: op.description} : {})})
            : await api('POST', '/tags/aliases', {alias: op.tag, canonical: op.canonical});
          (op.kind === 'taxonomy' ? plan.report.taxonomy_added : plan.report.aliased)
            .push({tag: op.tag, ...(op.canonical ? {canonical: op.canonical} : {}), linked: response.linked, auto_accepted: response.accepted});
        ...
        for (const record of op.item.records) {
          ...
          const response = await api('DELETE', `/sections/${record.record_id}/tags/${encodeURIComponent(op.tag)}`);
          plan.report.tags_stripped.push({tag: op.tag, record: record.file_path, remaining: response.tags});
        }
  ...
  for (const patch of plan.fmPatches) {
    try {
      await api('PATCH', `/sections/${patch.record}/fm`, {ops: [{op: 'add', path: '/related', value: patch.value}]});
    } catch (err) {
      if (!(err instanceof ApiError)) throw err;
  ...
  for (let i = 0; i < plan.batch.length; i += 100) {
    ...
    try {
      ...
      for (const result of response.results) if (result.error) failure(`resolve:${result.id}`, {status: 200, ...result.error});
    } catch (err) {
      if (!(err instanceof ApiError)) throw err;
